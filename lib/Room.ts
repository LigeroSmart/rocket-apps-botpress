...
export const closeChat = async (modify: IModify, read: IRead, rid: string) => {
    ...
    const closeChatMessage = await getAppSettingValue(read, AppSetting.botpressCloseChatMessage);

    const result = await modify.getUpdater().getLivechatUpdater()
                        .closeRoom(room, closeChatMessage ? closeChatMessage : DefaultMessage.DEFAULT_botpressCloseChatMessage);
...
export const performHandover = async (modify: IModify, read: IRead, rid: string, visitorToken: string,
    ...
    const targetDepartment = targetDepartmentName || await getAppSettingValue(read, AppSetting.botpressDefaultHandoverDepartment);
    if (!targetDepartment) {
